							...
							if(isset($survey['respondent_id_c']) && isset($survey['dwhs_key']) && isset($survey['email_address_c'])){
								if(!empty($survey['respondent_id_c']) && !empty($survey['dwhs_key']) && !empty($survey['email_address_c'])){
									...
									if($field=='dwhs_key'){
										...
										$formattedDate = $date->format("Y-m-d");// Desired format
										$value = $formattedDate;
...
function getValidDocuments(){
	...
	$get_doc->where()->addRaw("dr.deleted=0  AND survey_processing_c='1' AND source_c='Survey Monkey' AND dr.file_ext='csv' AND DATE(d.date_entered)= current_date()");
